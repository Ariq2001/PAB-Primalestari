    ...
    <div class="container-fluid">
      <div class="row">
		...
		<button type="button" data-toggle="modal" data-target="#modal-lg"><i class="fas fa-user-plus" > Tambah Data</i></button>
              <p>
              <?php $kodewilayah = mysqli_query($koneksi, "select * from tb_wilayah");?>
              <select class="from-select" aria-label="Default select example" style="width: 15%;">
                <option selected>kode pelanggan..</option>
                <?php while($row = mysqli_fetch_array($kodewilayah)) {?>
                <option value="<?php $row['kodewilayah']?>"><?php echo $row['kodewilayah']?></option>
                <?php } ?>
              </select>
                <br></br>
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php
                  $no = 0;
                  $query = mysqli_query($koneksi,"SELECT * FROM tb_dataplgn order by rt");
